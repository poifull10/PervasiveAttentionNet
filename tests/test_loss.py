...
class TestSeqCrossEntropy(unittest.TestCase):
    ...
    def test_seq_cross_entropy(self):
        ...
        p = Variable(np.array([[1.0, 1.0, 0., 0., 0., 1, 1]], dtype=np.float32))
        self.assertEqual(seq_cross_entropy(p, dst_seq, 1e-10).data, 0)

        dst_seq = np.array([[2, 10, 3, 1, 2, -1, -1]], dtype=np.int32)
        p = Variable(np.array([[0.5, 1.0, 0.5, 0., 0., 0.5, 0.3]], dtype=np.float32))
        self.assertTrue(seq_cross_entropy(p, dst_seq, 1e-10).data  <  - 2 * 0.5 * np.log(0.5) + 1e-3)
        self.assertTrue(seq_cross_entropy(p, dst_seq, 1e-10).data  >  - 2 * 0.5 * np.log(0.5) - 1e-3)
